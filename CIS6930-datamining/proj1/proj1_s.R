...
c45_model <- function(data_train){
  fit <- train(data_train[,c(1,2,3)], data_train[,4], method="J48",tuneLength = 10, preProcess=c("center", "scale"), trControl=trctrl, metric="Accuracy")
  return(fit)
}

#ripper
RIP_model <- function(data_train){
  fit <- train(data_train[,c(1,2,3)], data_train[,4], method="JRip", tuneLength = 10, preProcess=c("center", "scale"), trControl=trctrl,metric="Accuracy")
  return(fit)
}
...
main <- function(){
  ...
  #run exp 5 times
  for (i in 1:5) {
    print(sprintf("The %s round of analysis...", i))
	...
	for(j in 1:4){
      ...
      rc <- res_set[2]
      pc <- res_set[3]
      print(sprintf("The results for %s method: accuracy: %.3f; percision: %.3f; recall: %.3f", mn, res_set[1], res_set[3], res_set[2]))
  ...
  for(i in 1:4){
    ...
    print(sprintf("method name: %s;  averaged accuracy: %.3f;  accuracy standard deviation: %.3f", umethod, avg[i], msd[i]))
